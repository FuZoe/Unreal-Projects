...
UTP_WeaponComponent::UTP_WeaponComponent()
{
	...
	MuzzleOffset = FVector(100.0f, -8.0f, 30.0f);
}
...
void UTP_WeaponComponent::Fire()
{
	...
	if (GetWorld()->GetGameState<ABigProjectGameState>()->GameState != EGameState::OnGame)
	{
		return;
	}

	if (!Character->OnBulletChange(-1))
	{
		if (FireSound != nullptr)
		{
			UGameplayStatics::PlaySoundAtLocation(this, UnFireSound, Character->GetActorLocation());
		}
		return;
	}
	...
	if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			...
			Character->SpawnBullet(SpawnTransform, Character);
		}
	}
...
bool UTP_WeaponComponent::AttachWeapon(ABigProjectCharacter* TargetCharacter)
{
